...
if [ ! -b $SD_CARD_DEVICE"1" ]; then
    echo no device $SD_CARD_DEVICE"1"
    exit 0
fi
...
if  [ ! -f "$IN_BOOT_FILES/config.txt.rom" ]; then
    echo "no <boot file dir>/config.txt.rom"
    exit 0
fi

mounted=`grep $SD_CARD_DEVICE /proc/mounts`
if [ $? -eq 0 ]; then
    echo "$SD_CARD_DEVICE mounted"
    exit 0
fi
